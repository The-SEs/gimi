...
from users.permissions import IsAdminRole, IsStudent, IsCounselor, IsSecurity, IsNurse
...
def make_student(username, email, password="SuperSecret123!"):
    """Helper: create a user with the STUDENT role."""
...
class BaseTestCase(APITestCase):
    ...
    def setUp(self):
        self.user       = make_student("student123", "student@iacademy.edu.ph")
        self.other_user = make_student("other456",   "other@iacademy.edu.ph")
...
class JournalListCreateTests(BaseTestCase):
    ...
    def test_create_journal_entry_success(self):
        """An authenticated STUDENT can create a journal entry."""
    ...
    def test_create_journal_entry_without_title(self):
        """Title is optional; entry should still be created with a blank title."""
        response = self.client.post(self.url, {"content": "No title today."})

        self.assertEqual(response.status_code, status.HTTP_201_CREATED)
        self.assertEqual(response.data["title"], "")

    def test_create_journal_entry_missing_content_fails(self):
        """Content is required; omitting it must return 400."""
    ...
    def test_create_journal_returns_status_and_ai_response(self):
        """Response must include status and ai_response for safe entries."""
    ...
    def test_list_returns_only_own_journals(self):
        """A STUDENT should only see their own journal entries."""
...
class JournalDetailTests(BaseTestCase):
    ...
    def test_retrieve_own_journal_entry(self):
        """A STUDENT can fetch one of their own journal entries."""
    ...
    def test_update_own_journal_entry(self):
        """A STUDENT can update their own journal entry (PUT)."""
    ...
    def test_partial_update_own_journal_entry(self):
        """A STUDENT can partially update their own journal entry (PATCH)."""
    ...
    def test_update_returns_status_and_ai_response(self):
        ...
        response = self.client.put(
            self.detail_url, {"title": "Updated", "content": "Updated content."}
        )
        self.assertEqual(response.data["status"], "success")
        self.assertIn("ai_response", response.data)
    ...
    def test_delete_own_journal_entry(self):
        """A STUDENT can delete their own journal entry."""
    ...
    def test_cannot_access_other_users_journal_entry(self):
        """A STUDENT must not retrieve another student's journal entry — returns 404."""
        self._authenticate(self.other_user)
        response = self.client.get(self.detail_url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_cannot_update_other_users_journal_entry(self):
        """A STUDENT must not update another student's journal entry — returns 404."""
        self._authenticate(self.other_user)
        response = self.client.put(self.detail_url, {"title": "X", "content": "X"})
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)

    def test_cannot_delete_other_users_journal_entry(self):
        """A STUDENT must not delete another student's journal entry — returns 404."""
        self._authenticate(self.other_user)
        response = self.client.delete(self.detail_url)
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
...
class DailyMoodListCreateTests(BaseTestCase):
    ...
    def test_create_daily_mood_with_all_valid_states(self):
        """All four mood states (HP, SD, AN, CM) must be accepted."""
        for i, state in enumerate(["HP", "SD", "AN", "CM"]):
            user = make_student(f"user_{i}", f"user{i}@iacademy.edu.ph")
    ...
    def test_update_mood_same_day_only_one_record_exists(self):
        """Multiple same-day POSTs must result in exactly one record with the latest state."""
        self.client.post(self.url, {"state": "HP"})
    ...
    def test_list_daily_moods_returns_own_entries_only(self):
        """A STUDENT should only see their own daily mood entries."""
        self.client.post(self.url, {"state": "CM"})
