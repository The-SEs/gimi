...
export default function MoodBoard({ className = "" }: MoodBoardProps) {
  ...
  const loadTodayMood = async () => {
    try {
      setStatus("loading");
      const { data } = await api.get<DailyMoodResponse[]>(
        "/api/wellness/daily-moods/",
      );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("daily-mood:updated", loadTodayMood);
  }, []);
  ...
  const handleMoodSelection = async (mood: MoodEntry) => {
    ...
    if (
      isLoggedToday &&
      !window.confirm("Are you sure you want to change your mood for the day?")
    )
      return;

    try {
      setSaving(true);
      const { data } = await api.post<DailyMoodResponse>(
        "/api/wellness/daily-moods/",
        {
          state: mood.state,
        },
  ...
  if (status === "loading") {
    return (
      <div className={`relative w-full font-varela ${className}`}>
        <div className="bg-white py-5 px-7 rounded-3xl flex flex-col gap-4 shadow-sm animate-pulse h-full">
  ...
  return (
    <div className={`relative w-full font-varela ${className}`}>
      ...
      <div className="bg-white py-5 px-7 rounded-3xl flex flex-col gap-4 shadow-sm h-full">
        <h2 className="text-2xl text-GIMI-blue font-bold">Mood of the Day</h2>

        <div className="border border-dashed border-blue-100 rounded-xl flex flex-col gap-1 items-center py-5">
        ...
        {saving && (
          <p className="text-xs text-blue-300 text-center px-2 animate-pulse">
            Saving…
          </p>
        )}
      </div>
